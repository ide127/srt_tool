...
class SrtToolApp(tk.Tk):
    ...
    def _load_prompt(filename):
        ...
        try:
            ...
            return "You are a helpful assistant that translates subtitles into natural Korean."
    ...
    def create_log_box(self):
        ...
        clear_button = ttk.Button(
            controls_frame, text="로그 지우기", command=self._clear_log
        )
        clear_button.pack(side="right")

        # 로그 텍스트 프레임
        log_text_frame = ttk.LabelFrame(
            log_container, text="처리 로그", padding=(10, 5)
        )
        ...
        self.log_text.tag_configure(
            "ERROR", foreground="red", font=("Courier New", 9, "bold")
        )
    ...
    def _split_single_srt(self, srt_path, time_dir, sentence_dir):
        ...
        try:
            ...
            with (
                open(time_file_path, "w", encoding="utf-8") as tf,
                open(sentence_file_path, "w", encoding="utf-8") as sf,
            ):
                for block in blocks:
    ...
    def _translate_single_file(self, txt_path):
        ...
        try:
            ...
            if not original_content.strip():
                self.log_queue.put(
                    (f"'{filename}' 파일이 비어 있어 건너뜁니다.", "WARNING", False)
                )
                return True
        except Exception as e:
        ...
        for model_name in models_to_try:
            self.log_queue.put((f"'{model_name}' 모델로 번역 시도...", "INFO", False))
            ...
            try:
                ...
                if process.returncode != 0:
                    self.log_queue.put(
                        (f"'{model_name}' 모델 실행 실패: {stderr}", "ERROR", False)
                    )
                    continue  # 다음 모델로 재시도
                ...
                is_valid, error_line_idx = SrtToolApp._validate_translation_format(
                    translated_content
                )
                if is_valid:
                    is_sequential, expected_num = (
                        SrtToolApp._validate_sequential_numbering(translated_content)
                    )
                    if is_sequential:
                        ...
                        self.log_queue.put(
                            (
                                f"번역 성공 및 형식/순서 확인: {filename} (모델: {model_name})",
                                "INFO",
                                False,
                            )
                        )
                        ...
                        self.log_queue.put(
                            (
                                f"'{model_name}' 모델 번역 결과 순서 오류 발생. (예상: {expected_num}, 파일: {filename})",
                                "WARNING",
                                False,
                            )
                        )
                    ...
                    self.log_queue.put(
                        (
                            f"'{model_name}' 모델 번역 결과 형식 오류 발생.",
                            "WARNING",
                            False,
                        )
                    )
                ...
                self.log_queue.put(
                    (
                        f"예상치 못한 번역 오류 ({filename}, 모델: {model_name}): {e}",
                        "ERROR",
                        False,
                    )
                )
        ...
        self.log_queue.put(
            (
                f"번역 최종 실패: 모든 모델({', '.join(models_to_try)}) 시도 후에도 형식이 올바르지 않음. ({filename})",
                "ERROR",
                False,
            )
        )
    ...
    def _merge_single_srt(self, time_file_path, sentence_file_path, output_srt_path):
        ...
        try:
            ...
            if not os.path.exists(time_file_path) or not os.path.exists(
                ...
                self.log_queue.put(
                    ("병합에 필요한 파일이 없습니다. 건너뜁니다.", "WARNING", False)
                )
                return False
            ...
            for t_chunk, s_chunk in zip(time_chunks, sentence_chunks):
                ...
                text_lines = [
                    ...
                    for line in (
                        s_lines[1:]
                        if s_lines[0].isdigit() and s_lines[0] == original_number
                        else s_lines
                    )
                    if line.strip()
                ]
                ...
                if not text_lines:
                    # 텍스트가 없는 블록 (예: [music playing] 후 번역에서 제거된 경우)
                    # 원본 넘버링은 유지하되, 시간과 텍스트는 비워둠 (혹은 다른 정책 적용 가능)
                    # 여기서는 그냥 건너뛰어 최종 파일에서 제거되도록 함
                    continue
                ...
                if len(text_lines) == 1:
                    ...
                    try:
                        ...
                        start_td = SrtToolApp._time_str_to_timedelta(start_time_str)
                        end_td = SrtToolApp._time_str_to_timedelta(end_time_str)
                        total_duration_td = end_td - start_td

                        total_len = sum(len(line) for line in text_lines)
                        if total_len == 0:
                            continue  # 분배할 텍스트가 없으면 건너뛰기
                        ...
                        for i, line in enumerate(text_lines):
                            ...
                            # 마지막 라인은 오차 방지를 위해 원래 종료 시간으로 설정
                            if i == len(text_lines) - 1:
                                line_end_td = end_td

                            new_start_str = SrtToolApp._timedelta_to_time_str(
                                current_start_td
                            )
                            new_end_str = SrtToolApp._timedelta_to_time_str(line_end_td)
                        ...
                        self.log_queue.put(
                            (
                                f"자막 시간 분배 중 오류 ({filename}, 블록: {original_number}): {e}",
                                "ERROR",
                                False,
                            )
                        )
                        # 오류 발생 시 원본 블록을 그대로 추가하고 계속 진행
                        srt_output.append(
                            f"{new_block_counter}\n{time_line}\n{'\n'.join(text_lines)}\n"
                        )
                        new_block_counter += 1
    ...
    def _backup_failed_srt(self, srt_path, base_dir):
        ...
        try:
            ...
            self.log_queue.put(
                (
                    f"원본 SRT를 'failed_srt' 폴더에 백업: {os.path.basename(srt_path)}",
                    "WARNING",
                    False,
                )
            )
    ...
    def _execute_split_all(self, dir_path):
        ...
        if not srt_files:
            self.log_queue.put(
                ("폴더에서 SRT 파일을 찾을 수 없습니다.", "ERROR", False)
            )
    ...
    def _execute_merge_all(self, dir_path):
        ...
        self.log_queue.put(
            (f"병합 작업 시작... (대상 폴더: {dir_path})", "INFO", False)
        )
        time_dir = os.path.join(dir_path, "txtWithTime")
        sentence_dir = os.path.join(dir_path, "txtWithSentence")
    ...
    def _execute_adv_shift(self, dir_path, start_num, offset):
        ...
        self.log_queue.put(
            (f"시간 일괄 조절 시작... (대상 폴더: {dir_path})", "INFO", False)
        )
        srt_files = [f for f in os.listdir(dir_path) if f.lower().endswith(".srt")]
        if not srt_files:
            self.log_queue.put(
        ...
        self.log_queue.put(
            (
                f"총 {total_files}개 중 {success_count}개 파일 시간 조절 완료. 'timeShiftedSrt' 폴더를 확인하세요.",
                "INFO",
                False,
            )
        )
    ...
    def _execute_translation_all(self, sentence_dir):
        ...
        self.log_queue.put(
            (f"총 {len(txt_files)}개의 파일에 대한 번역을 시작합니다.", "INFO", False)
        )
        total_files = len(txt_files)
        self.progress_var.set(0)
        success_count = 0
    ...
    def run_translation_wrapper(self):
        ...
        if not os.path.isdir(sentence_dir):
            messagebox.showerror(
                "폴더 없음", "`txtWithSentence` 폴더를 찾을 수 없습니다."
            )
            return
    ...
    def _execute_one_click_workflow(self, dir_path):
        ...
        self.log_queue.put(("🚀 원클릭 전체 작업을 시작합니다.", "INFO", False))
        ...
        for i, srt_file in enumerate(srt_files, 1):
            ...
            self.progress_var.set(
                (i / total_files) * 95
            )  # 95% for processing, 5% for finalization
            srt_path = os.path.join(dir_path, srt_file)
            base_name = os.path.splitext(srt_file)[0]
