...
def _translate_single_file(txt_path, instruction_prompt, log_queue):
    ...
    for model_name in models_to_try:
        ...
        try:
            ...
            is_valid, error_line_idx = utils._validate_translation_format(
                translated_content
            )
            if is_valid:
                is_sequential, expected_num = utils._validate_sequential_numbering(
                    translated_content
                )
                if is_sequential:
                ...
                log_queue.put(
                    (f"'{model_name}' 모델 번역 결과 형식 오류 발생.", "WARNING", False)
                )
